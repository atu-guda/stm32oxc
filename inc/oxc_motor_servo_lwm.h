#ifndef _OXC_MOTOR_SERVO_LWM_H
#define _OXC_MOTOR_SERVO_LWM_H
...
class MotorServoLWM : public MotorServoLWMBase {
  public:
   MotorServoLWM( PwmCtl &pwmc_, std::size_t ch_ )
     : MotorServoLWMBase( pwmc_, ch_ )
   {
     props = canSetX | canBreak;
   }
   virtual ReturnCode set_v( float v ) override { return rcErr; }
   virtual ReturnCode set_x( float x ) override { return set_xv( x ); }
