...
void HAL_SPI_MspInit( SPI_HandleTypeDef *hspi )
{
  ...
  BOARD_SPI_DEFAULT_PIN_SCK. enableClk();
  if constexpr ( BOARD_SPI_DEFAULT_PIN_SCK.portNum() != BOARD_SPI_DEFAULT_PIN_MISO.portNum() ) {
    BOARD_SPI_DEFAULT_PIN_MISO.enableClk();
  }
  if constexpr ( BOARD_SPI_DEFAULT_PIN_MISO.portNum() != BOARD_SPI_DEFAULT_PIN_MOSI.portNum() ) {
    BOARD_SPI_DEFAULT_PIN_MOSI.enableClk();
  }
...
void HAL_SPI_MspDeInit( SPI_HandleTypeDef* hspi )
{
  if( hspi->Instance == BOARD_SPI_DEFAULT ) {
    ...
    BOARD_SPI_DEFAULT_PIN_SCK .cfgIn();
    BOARD_SPI_DEFAULT_PIN_MISO.cfgIn();
    BOARD_SPI_DEFAULT_PIN_MOSI.cfgIn();
  }
}
