...
void TypeInit(Picoc *pc)
{
  ...
  pc->IntPtrType = TypeAdd(pc, NULL, &pc->IntType, TypePointer, 0, pc->StrEmpty, sizeof(void *), PointerAlignBytes);
  pc->IntArrayType = TypeAdd(pc, NULL, &pc->IntType, TypeArray, 0, pc->StrEmpty, sizeof(char), (char *)&ia.y - &ia.x);
#ifndef NO_FP
  pc->FPPtrType = TypeAdd(pc, NULL, &pc->FPType, TypePointer, 0, pc->StrEmpty, sizeof(void *), PointerAlignBytes);
  pc->FPArrayType = TypeAdd(pc, NULL, &pc->FPType, TypeArray, 0, pc->StrEmpty, sizeof(char), (char *)&da.y - &da.x);
#endif
}
