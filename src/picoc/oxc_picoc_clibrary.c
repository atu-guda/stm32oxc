...
/* add a library */
void LibraryAdd(Picoc *pc, struct Table *GlobalTable __attribute((unused)), const char *LibraryName __attribute((unused)), struct LibraryFunction *FuncList)
{
  ...
  for (FPos = Format; *FPos != '\0'; FPos++)
  {
    if (*FPos == '%')
    {
      ...
      if (FormatType != NULL)
      {
        ...
        if (ArgCount >= NumArgs)
          PrintStr("xNX", Stream);   /* not enough parameters for format */
        else
        {
          ...
          if (NextArg->Typ != FormatType &&
            ...
            PrintStr("xTx", Stream);   /* bad type for format */
          else
          {
            switch (*FPos)
            {
